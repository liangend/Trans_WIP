...
As show in the following figure, red bars are cis effect, and blue bars are trans. Significant effects are bordered in black (p < 0.05/245). Significant trans effects seem to always have the same direction. For loci 5000, there are two genes (F10 and F7) located in cis regions of trans-pQTL.
...
```
```{r echo=F, fig.align='center', fig.cap="Trans pQTL effect directions (19_cis_trans_direction.R)"}
knitr::include_graphics(c("assets/pqtl_cis_trans_dir3.png"), error = FALSE)
```
